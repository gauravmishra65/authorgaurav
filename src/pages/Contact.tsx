      ...
      <section className="bg-ink text-ivory paper-grain">
        ...
        <div className="mx-auto max-w-5xl px-6 py-20 text-center">
          ...
          <p className="text-ivory/75 max-w-2xl mx-auto leading-relaxed">
            Whether you’re a reader, a fellow writer, or just passing through — I’d
        ...
        <div className="grid gap-12 md:grid-cols-[1.3fr_1fr]">
            ...
            {submitted ? (
              <div className="rounded-md border border-gold/40 bg-cream px-6 py-10 text-center">
                ...
                <p className="text-muted text-sm">
                  Thank you, {form.name.split(' ')[0]}. Your note is on its way — I’ll
                  reply soon
                  {form.circle ? ', and you’ve been added to the reader circle.' : '.'}
                </p>
                <button
                  onClick={() => {
                    setSubmitted(false);
                    setForm({ name: '', email: '', message: '', circle: false });
                  }}
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label htmlFor="name" className="label-caps text-muted block mb-2">
                    Name
                  </label>
                  <input
                    id="name"
                ...
                <div>
                  <label htmlFor="email" className="label-caps text-muted block mb-2">
                    Email
                  </label>
                  <input
                    id="email"
                ...
                <div>
                  <label htmlFor="message" className="label-caps text-muted block mb-2">
                    Message
                  </label>
                  <textarea
                    id="message"
                ...
                <label className="flex items-start gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={form.circle}
                    onChange={(e) => update('circle', e.target.checked)}
                    className="mt-1 h-4 w-4 accent-gold"
                  />
                  <span className="text-sm text-muted">
                    Add me to the reader circle — I’d like the free chapter and monthly
                    letters.
                  </span>
                </label>
                <button
          ...
          <aside className="space-y-8">
            <div className="rounded-md border border-gold/20 bg-cream p-6">
              ...
              <a
                href="mailto:hello@authorgaurav.com"
                className="inline-flex items-center gap-2 text-ink hover:text-gold transition-colors"
              >
                <Mail size={16} />
                hello@authorgaurav.com
              </a>
            </div>
              ...
              <ul className="space-y-3 text-sm">
                <li>
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 text-ink hover:text-gold transition-colors"
                  >
                    WriteTogetherHub <ExternalLink size={13} />
                  </a>
                </li>
                <li>
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 text-ink hover:text-gold transition-colors"
                  >
                    off-beat-love.com <ExternalLink size={13} />
                  </a>
                </li>
                <li>
                  <a
                    href="https://the-shadow-code.com"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 text-ink hover:text-gold transition-colors"
                  >
                    the-shadow-code.com <ExternalLink size={13} />
                  </a>
                </li>
              </ul>
            </div>
              ...
              <div className="flex gap-4">
                {['Instagram', 'Facebook', 'X', 'YouTube'].map((s) => (
                  <a
                    ...
                    href="#"
                    aria-label={s}
                    className="label-caps text-2xs text-muted hover:text-gold transition-colors border border-gold/25 rounded-full px-3 py-2"
                  >
                    {s}
                  </a>
                ))}
