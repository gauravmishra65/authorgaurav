...
export default function Blog() {
  ...
  const filtered =
    filter === 'All'
      ? posts
      : posts.filter((p) => p.category === (filter as BlogCategory));
      ...
      <section className="bg-ink text-ivory paper-grain">
        ...
        <div className="mx-auto max-w-5xl px-6 py-20 text-center">
          ...
          <p className="text-ivory/75 max-w-2xl mx-auto leading-relaxed">
            Notes on the writing life — craft, the stories behind the books, and
            reflections on the hymns that keep me grounded.
          </p>
        </div>
      </section>
          ...
          {filtered.map((p) => (
            ...
            >
              <div className={`h-44 bg-gradient-to-br ${p.gradient} relative`}>
                <span className="absolute bottom-3 left-3 label-caps text-2xs text-ivory/90 bg-ink/40 px-2.5 py-1 rounded-sm">
                  {p.category}
                </span>
              </div>
              <div className="p-5 flex flex-col flex-1">
                ...
                <h2 className="font-display text-xl text-ink mb-2 group-hover:text-gold transition-colors">
                  {p.title}
                </h2>
                ...
                <Link
                  to="/blog"
                  className="mt-4 inline-flex items-center gap-1.5 label-caps text-gold hover:text-ink transition-colors text-2xs"
                >
                  Read more
                  <ArrowRight size={13} />
                </Link>
