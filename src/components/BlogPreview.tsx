        ...
        {latest.map((p) => (
          <Link key={p.id} to={`/blog/${p.slug}`} className="group flex flex-col rounded-md border border-gold/15 bg-cream overflow-hidden shadow-luxury transition-all hover:-translate-y-1 hover:shadow-book-hover">
            ...
            <div className="p-5 flex flex-col flex-1">
              ...
              <h3 className="font-display text-lg text-ink mb-2 group-hover:text-gold-text transition-colors">{p.title}</h3>
              <p className="text-sm text-muted leading-relaxed line-clamp-3">{p.excerpt}</p>
            </div>
          </Link>
        ))}
      </div>
