...
// // Then forward to your contact-email service for a welcome note if desired.
  ...
  return (
    <section
      className={`relative overflow-hidden ${
        isDark ? 'bg-ink-soft text-ivory' : 'bg-gradient-to-r from-gold-lt/30 via-gold/15 to-gold-lt/30'
      }`}
    >
      ...
      <div className="mx-auto max-w-5xl px-6 py-14 text-center">
        ...
        <p className={`text-sm mb-7 ${isDark ? 'text-ivory/70' : 'text-muted'}`}>
          {subheading}
        </p>
        ...
        {submitted ? (
          <div className="max-w-md mx-auto rounded-md border border-gold/40 bg-cream/80 px-6 py-6 text-ink">
            ...
            <p className="text-sm text-muted mt-1">
              Check your inbox for the free chapter — and welcome to the reader circle.
            </p>
          </div>
        ) : (
          <form
          ...
          >
            ...
            <button
              type="submit"
              className="btn-caps btn-gold rounded-sm px-6 py-3 whitespace-nowrap"
            >
              Get the Chapter
            </button>
          </form>
